...
export const SuperAdminDashboardService = {
  getSummary: async () => {
    const [totalSocieties, activeSocieties, trialSocieties, lockedSocieties, totalSocietyAdmins] =
      await Promise.all([
        ...
        prisma.user.count({ where: { role: { name: 'SOCIETY_ADMIN' } } }),
      ]);

    return { totalSocieties, activeSocieties, trialSocieties, lockedSocieties, totalSocietyAdmins };
  },
  ...
  getRevenue: async () => {
    ...
    const [totalRevenueResult, thisMonthResult, lastMonthResult] = await Promise.all([
      ...
      prisma.payment.aggregate({
        where: { status: 'SUCCESS', paidAt: { gte: lastMonthStart, lte: lastMonthEnd } },
        _sum: { amount: true },
      }),
    ]);
  ...
  getSubscriptions: async () => {
    ...
    return { active, trial, grace, locked, expiringIn7Days };
  },
